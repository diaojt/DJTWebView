...
@implementation NSObject (DJTRuntime)
...
+ (NSArray *)djt_objectsWithArray:(NSArray *)array
{
    ...
    //获取属性列表数组
    NSArray *propertyList = [self djt_propertysList];
 ...
 *  程序运行的时候,类的属性不会变化
 */
const void *djt_propertyListKey = @"djt_propertyListKey";
+ (NSArray *)djt_propertysList
{
    NSArray *result = objc_getAssociatedObject(self, djt_propertyListKey);
    if (result != nil) {
        return result;
    }
    ...
    /**⚠️注意：class_copyPropertyList 底层是 c语言，所以我们一定要记得释放properties */
    free(list);
    
    //保存属性数组对象
    objc_setAssociatedObject(self, djt_propertyListKey, arrayM, OBJC_ASSOCIATION_COPY_NONATOMIC);
    return objc_getAssociatedObject(self, djt_propertyListKey);
}


#pragma mark ======== 获取本类所有“方法”的数组 ========
const void *djt_methodListKey = "djt_methodListKey";
+ (NSArray *)djt_methodList
{
    //1.使用运行时动态添加属性
    NSArray *methodsList = objc_getAssociatedObject(self, djt_methodListKey);
    ...
    //5.保存方法的数组对象
    objc_setAssociatedObject(self, djt_methodListKey, arrayM, OBJC_ASSOCIATION_COPY_NONATOMIC);
    
    return  objc_getAssociatedObject(self, djt_methodListKey);
}


#pragma mark ======== 获取本类所有 ‘成员变量‘ 的数组 <用来调试> ========
/** 获取当前类的所有成员变量 */
const char *djt_ivarListKey = "djt_ivarListKey";
+ (NSArray *)djt_ivarList
{
    
    // 1. 查询根据key 保存的成员变量数组
    NSArray *ivarList = objc_getAssociatedObject(self, djt_ivarListKey);
    ...
    objc_setAssociatedObject(self, djt_ivarListKey, arrayM, OBJC_ASSOCIATION_COPY_NONATOMIC);
    
    return objc_getAssociatedObject(self, djt_ivarListKey);
}

#pragma mark - 获取本类所有 ‘协议‘ 的数组
/** 用来获取动态保存在关联对象中的协议数组 |运行时的关联对象根据key动态取值| */
const char *djt_protocolListKey = "djt_protocolListKey";
+ (NSArray *)djt_protocolList {
    NSArray *protocolList = objc_getAssociatedObject(self, djt_protocolListKey);
    if (protocolList != nil)
    {
        return protocolList;
    }
    ...
    free(protocolLists);
    // 将保存 协议的数组动态添加到 关联对象
    objc_setAssociatedObject(self, djt_protocolListKey, arrayM, OBJC_ASSOCIATION_COPY_NONATOMIC);
    
    return objc_getAssociatedObject(self, djt_protocolListKey);
}
